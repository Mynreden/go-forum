...
func TestPostStorage_CreatePost(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _ = range tc.post.Categories {
			...
			po := &domain.CreatePostDTO{
				Title:      tc.post.Title,
				Content:    tc.post.Content,
				Categories: tc.post.Categories,
				Author:     tc.post.ID,
				AuthorName: tc.post.AuthorName,
				ImageFile:  nil,
			}
			repo := post.NewPostStorage(db)
			service := postService.NewPostService(repo)
			id, err := service.CreatePost(po)
...
func TestCreatePostWithImage(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			id, err := service.CreatePostWithImage(po)
...
func TestGetAllPosts(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			posts, err := service.GetAllPosts(tc.offset, tc.limit)
